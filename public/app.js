...
function lobby(){
  screen.innerHTML=`<div class=box><h2>Online spielen</h2><input id=name placeholder="Dein Name"><button onclick="create()">Raum erstellen</button><hr><input id=code placeholder="Raumcode"><button onclick="join()">Raum beitreten</button></div>`;
}
...
function bid(head,p){
  const submitted=!!state.myBidSubmitted;
  screen.innerHTML=head+`<div class=box><h2>🪙 Gebot</h2>${submitted?
    `<div class="bid-confirmed">✓ Dein Gebot wurde abgegeben und deine gesetzten Tokens wurden ausgegeben.</div><p class=muted>Warte auf das Gebot deines Gegners…</p>`:
    `<p>Du hast <b>${p.tokens}</b> Tokens übrig. Dein Einsatz wird sofort als ausgegeben gezählt.</p><input id=bid type=number min=0 max=${p.tokens} value=0><button id=bidButton onclick="submitBid()">Gebot abgeben</button>`
  }</div>`+notesButton();
}
function submitBid(){
...
function choice(head){
  ...
  screen.innerHTML=head+(active?`<div class=box><h2>Du hast gewonnen – wähle!</h2><button onclick="questions()">❓ Frage</button> <button class=danger onclick="truth()">🟥 Wahrheit</button><div id=actions></div></div>`:`<div class=box>Dein Gegner entscheidet…</div>`)+notesButton();
}
...
function ask(type,arg){
  const q={type};
  if(type==="sum_positions"){const x=prompt("3 Positionen, z.B. 1,4,7");const a=x?.split(",").map(n=>Number(n.trim()));if(!a||a.length!==3||a.some(n=>!Number.isInteger(n)||n<1||n>8))return err("Bitte genau 3 Positionen von 1 bis 8 eingeben.");q.positions=a.map(n=>n-1);}
  else if(arg==="Farbe"){const s=prompt("♠ ♥ ♦ oder ♣");if(!s||!suits.includes(s))return err("Bitte eine gültige Farbe wählen.");q.suit=s;}
